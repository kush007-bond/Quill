...
export function findTables(state: EditorState): TableRange[] {
  const doc = state.doc;
  ...
  class {
    ...
    buildDecorations(view: EditorView): DecorationSet {
      ...
      for (const t of tables) {
        // When the cursor is inside the range, CM silently drops block replace
        // decorations. Skip it explicitly so raw markdown is shown instead.
        if (cursorPos >= t.from && cursorPos <= t.to) continue;
...
/**
 * Helper to insert a fresh table at the current cursor position.
 */
